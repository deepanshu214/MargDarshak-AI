...
const Chatbot: React.FC<ChatbotProps> = ({ user, updateProfile }) => {
  ...
  return (
    <div className="p-3 sm:p-6 md:p-8 h-full flex flex-col justify-center animate-in fade-in duration-500">
      <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl w-full mx-auto bg-white/95 backdrop-blur-md rounded-3xl shadow-xl border border-stone-200/80 overflow-hidden">
        
        {/* Mentor Header */}
        <div className="p-4 sm:p-5 bg-gradient-to-r from-emerald-900 via-emerald-800 to-slate-900 text-white flex items-center justify-between shadow-md shrink-0 border-b border-emerald-950">
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 bg-white/10 rounded-2xl flex items-center justify-center backdrop-blur-sm shadow-inner border border-white/10">
              <Sparkles className="w-5 h-5 text-amber-300" />
            </div>
            <div>
              <h2 className="font-black text-base tracking-tight flex items-center gap-2 font-serif">
                MargDarshak AI Mentor
                <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span>
              </h2>
              <p className="text-[10px] font-bold text-emerald-200 uppercase tracking-wider font-mono">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="p-2 hover:bg-white/10 rounded-xl transition-all text-emerald-200 hover:text-white"
              title="Reset Chat"
            >
              <Trash2 className="w-4 h-4" />
            </button>
            <div className="bg-white/10 px-3 py-1 rounded-xl text-xs font-black uppercase text-amber-300 border border-white/10 font-mono">
              {lang}
            </div>
          </div>
        </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-[#faf7f0]/60">
            ...
            <div key={idx} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex max-w-[85%] md:max-w-[75%] gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${
                  isUser ? 'bg-emerald-800 text-amber-300' : 'bg-white text-emerald-800 border border-stone-200'
                }`}>
                  {isUser ? <User className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
                </div>

                {/* Bubble */}
                <div className={`rounded-2xl p-4 sm:p-5 text-xs sm:text-sm leading-relaxed shadow-xs ${
                  isUser 
                  ...
                  {!isUser && msg.sources && msg.sources.length > 0 && (
                    <div className="mt-4 pt-3 border-t border-stone-100">
                      <p className="text-[9px] font-black text-stone-400 uppercase tracking-wider mb-2 font-mono">
                        Official Citations:
                      </p>
                        ...
                        {msg.sources.map((s: any, sIdx: number) => (
                          s.web && (
                            <a 
                              ...
                              target="_blank" 
                              rel="noreferrer"
                              className="text-[10px] text-emerald-800 font-bold hover:underline bg-emerald-50 px-2 py-0.5 rounded-lg border border-emerald-200 flex items-center gap-1"
                            >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition-colors flex items-center gap-1 text-[10px] font-bold ${
                            speakingMsgIdx === idx ? 'text-emerald-800 bg-emerald-50 font-black' : 'hover:text-stone-600'
                          }`}
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg hover:text-stone-600 transition-colors flex items-center gap-1 text-[10px] font-bold"
                          title="Copy message"
                        >
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="flex gap-3 items-center">
              <div className="w-8 h-8 rounded-xl bg-white border border-stone-200 flex items-center justify-center text-emerald-800 shadow-xs">
                <Loader2 className="w-4 h-4 animate-spin" />
              </div>
              <div className="bg-white px-4 py-2.5 rounded-2xl border border-stone-200 text-stone-400 text-xs font-bold italic shadow-xs">
      ...
      {messages.length <= 2 && (
        <div className="px-4 py-2 bg-stone-50 border-t border-stone-200 flex items-center gap-2 overflow-x-auto shrink-0">
          <span className="text-[10px] font-black uppercase text-stone-400 shrink-0 font-mono">Suggestions:</span>
          {chips.map((chip, i) => (
            <button
              key={i}
              onClick={() => handleSend(chip)}
              className="px-3 py-1.5 bg-white hover:bg-emerald-50 hover:text-emerald-800 border border-stone-200 rounded-xl text-xs font-bold text-stone-600 whitespace-nowrap transition-colors shadow-2xs"
            >
              {chip}
            </button>
          ))}
      ...
      {/* Bottom Input Area */}
      <div className="p-3 sm:p-4 bg-white border-t border-stone-200 sticky bottom-0 z-10 shrink-0">
        <div className="flex items-center gap-2 bg-stone-100 rounded-2xl p-1.5 focus-within:ring-2 focus-within:ring-emerald-600 focus-within:bg-white transition-all">
          <input 
            type="text" 
          ...
          <button
            ...
            className={`p-2.5 rounded-xl transition-all ${
              isListening 
                ...
                : 'text-stone-400 hover:text-emerald-700 hover:bg-stone-200'
            }`}
          ...
          <button 
            ...
            className={`p-2.5 rounded-xl transition-all ${
              input.trim() && !isTyping 
                ? 'bg-emerald-800 text-amber-300 shadow-sm border border-amber-400/30 hover:bg-emerald-900 active:scale-95' 
